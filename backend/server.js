...
const privateKey = fs.readFileSync('/etc/letsencrypt/live/casoscomplejos.duckdns.org/privkey.pem', 'utf8');
const certificate = fs.readFileSync('/etc/letsencrypt/live/casoscomplejos.duckdns.org/cert.pem', 'utf8');
const ca = fs.readFileSync('/etc/letsencrypt/live/casoscomplejos.duckdns.org/chain.pem', 'utf8');
...
const { processAndUpload } = require('./driveUploader');
...
const db = new sqlite3.Database(dbPath, (err) => {
    ...
    else console.log('Conectado a la base de datos (pacientes.db).');
});

// --- MIGRACIONES SUAVES ---
const requiredColumns = [
    { name: 'vinculo_titular', type: 'TEXT' },
    { name: 'observaciones', type: 'TEXT' },
    { name: 'sexo', type: 'TEXT' },
    { name: 'titular_dni', type: 'TEXT' },
    { name: 'titular_nombre', type: 'TEXT' }
];

function ensureColumnsExist() {
    db.all("PRAGMA table_info(pacientes)", (err, rows) => {
        if (err) {
            console.error('No se pudo obtener info de la tabla pacientes:', err.message);
            return;
        }
        const existing = rows.map(r => r.name);
        requiredColumns.forEach(col => {
            if (!existing.includes(col.name)) {
                const sql = `ALTER TABLE pacientes ADD COLUMN ${col.name} ${col.type}`;
                db.run(sql, [], (alterErr) => {
                    if (alterErr) console.error(`Error al añadir columna ${col.name}:`, alterErr.message);
                    else console.log(`Columna añadida: ${col.name}`);
                });
            }
        });
    });
...
app.get('/api/prestadores', checkSession, (req, res) => {
    const sql = `
...
app.get('/api/anios-prestaciones', checkSession, (req, res) => {
    const sql = `
        SELECT DISTINCT 
            CASE 
                WHEN fecha LIKE '__/__/____' THEN substr(fecha, -4)
                WHEN fecha LIKE '_/__/____' THEN substr(fecha, -4)
                ELSE NULL
            END AS anio
        FROM prestaciones 
...
app.get('/api/busqueda-avanzada', checkSession, (req, res) => {
    ...
    if (afiliado && afiliado.trim()) {
        ...
        if (/^\d+$/.test(valor)) {
            ...
            wheres.push("REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(p.nombre), 'Á', 'A'), 'É', 'E'), 'Í', 'I'), 'Ó', 'O'), 'Ú', 'U'), 'Ñ', 'N'), 'Ü', 'U'), 'À', 'A'), 'È', 'E'), 'Ò', 'O'), 'á', 'a'), 'é', 'e'), 'í', 'i'), 'ó', 'o'), 'ú', 'u'), 'ñ', 'n'), 'ü', 'u'), 'à', 'a'), 'è', 'e'), 'ò', 'o') LIKE ? || '%'");
    ...
    if (localidad && localidad.trim()) {
        ...
        wheres.push("p.localidad IS NOT NULL AND REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(p.localidad), 'Á', 'A'), 'É', 'E'), 'Í', 'I'), 'Ó', 'O'), 'Ú', 'U'), 'Ñ', 'N'), 'Ü', 'U'), 'À', 'A'), 'È', 'E'), 'Ò', 'O'), 'á', 'a'), 'é', 'e'), 'í', 'i'), 'ó', 'o'), 'ú', 'u'), 'ñ', 'n'), 'ü', 'u'), 'à', 'a'), 'è', 'e'), 'ò', 'o') LIKE '%' || ? || '%'");
        params.push(localidadNorm);
    }

    // Filtro: Condición
    if (condicion && condicion !== '') {
        wheres.push("p.condicion = ?");
        params.push(condicion);
    }

    // Filtro: Tipo Afiliado
    if (tipo_afiliado && tipo_afiliado !== '') {
        wheres.push("UPPER(p.tipo_afiliado) = UPPER(?)");
    ...
    if (prestador || anio) {
        ...
        if (prestador && prestador.trim()) {
            ...
            wheres.push("REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(pr.prestador), 'Á', 'A'), 'É', 'E'), 'Í', 'I'), 'Ó', 'O'), 'Ú', 'U'), 'Ñ', 'N'), 'Ü', 'U'), 'À', 'A'), 'È', 'E'), 'Ò', 'O'), 'á', 'a'), 'é', 'e'), 'í', 'i'), 'ó', 'o'), 'ú', 'u'), 'ñ', 'n'), 'ü', 'u'), 'à', 'a'), 'è', 'e'), 'ò', 'o') = ?");
        ...
        if (rows.length === 0) {
            ...
            function buscarAlternativa(index) {
                ...
                if (filtrosAlt.afiliado && filtrosAlt.afiliado.trim()) {
                    ...
                    if (/^\d+$/.test(valor)) {
                        wheresAlt.push("p.dni = ?");
                        paramsAlt.push(valor);
                    } else {
                        const normalizado = valor.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase();
                        wheresAlt.push("REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(p.nombre), 'Á', 'A'), 'É', 'E'), 'Í', 'I'), 'Ó', 'O'), 'Ú', 'U'), 'Ñ', 'N'), 'Ü', 'U'), 'À', 'A'), 'È', 'E'), 'Ò', 'O'), 'á', 'a'), 'é', 'e'), 'í', 'i'), 'ó', 'o'), 'ú', 'u'), 'ñ', 'n'), 'ü', 'u'), 'à', 'a'), 'è', 'e'), 'ò', 'o') LIKE ? || '%'");
                ...
                if (filtrosAlt.localidad && filtrosAlt.localidad.trim()) {
                    ...
                    wheresAlt.push("p.localidad IS NOT NULL AND REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(p.localidad), 'Á', 'A'), 'É', 'E'), 'Í', 'I'), 'Ó', 'O'), 'Ú', 'U'), 'Ñ', 'N'), 'Ü', 'U'), 'À', 'A'), 'È', 'E'), 'Ò', 'O'), 'á', 'a'), 'é', 'e'), 'í', 'i'), 'ó', 'o'), 'ú', 'u'), 'ñ', 'n'), 'ü', 'u'), 'à', 'a'), 'è', 'e'), 'ò', 'o') LIKE '%' || ? || '%'");
                    paramsAlt.push(localidadNorm);
                }

                if (filtrosAlt.condicion && filtrosAlt.condicion !== '') {
                    wheresAlt.push("p.condicion = ?");
                    paramsAlt.push(filtrosAlt.condicion);
                }

                if (filtrosAlt.tipo_afiliado && filtrosAlt.tipo_afiliado !== '') {
                    wheresAlt.push("UPPER(p.tipo_afiliado) = UPPER(?)");
                ...
                if (filtrosAlt.prestador || filtrosAlt.anio) {
                    ...
                    if (filtrosAlt.prestador && filtrosAlt.prestador.trim()) {
                        ...
                        wheresAlt.push("REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(pr.prestador), 'Á', 'A'), 'É', 'E'), 'Í', 'I'), 'Ó', 'O'), 'Ú', 'U'), 'Ñ', 'N'), 'Ü', 'U'), 'À', 'A'), 'È', 'E'), 'Ò', 'O'), 'á', 'a'), 'é', 'e'), 'í', 'i'), 'ó', 'o'), 'ú', 'u'), 'ñ', 'n'), 'ü', 'u'), 'à', 'a'), 'è', 'e'), 'ò', 'o') = ?");
                        paramsAlt.push(prestadorNorm);
                    }
                    if (filtrosAlt.anio && /^\d{4}$/.test(filtrosAlt.anio)) {
                        wheresAlt.push("pr.fecha LIKE '%/' || ?");
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        const sql = 'INSERT INTO usuarios (usuario, contrasena_hash, rol) VALUES (?, ?, ?)';
        db.run(sql, [usuario, contrasena_hash, rol], function(err) {
            if (err) {
...
app.post('/api/change-password', checkSession, async (req, res) => {
    ...
    try {
        ...
        const sql = 'UPDATE usuarios SET contrasena_hash = ?, debe_cambiar_pass = 0 WHERE usuario = ?';
        db.run(sql, [contrasena_hash, usuario], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            req.session.debe_cambiar_pass = 0;
...
app.get('/api/admin/users', checkSession, checkAdmin, (req, res) => {
    const sql = 'SELECT usuario, rol FROM usuarios WHERE usuario != ?';
    db.all(sql, [req.session.usuario], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(rows);
    });
});
...
app.post('/api/admin/reset-password', checkSession, checkAdmin, async (req, res) => {
    ...
    try {
        ...
        const sql = 'UPDATE usuarios SET contrasena_hash = ?, debe_cambiar_pass = 1 WHERE usuario = ?';
        db.run(sql, [contrasena_hash, usuario_a_resetear], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ message: `Contraseña de ${usuario_a_resetear} reseteada a '${PASS_RESET_GENERICA}'.` });
        });
    } catch (error) {
...
app.delete('/api/admin/user/:usuario', checkSession, checkAdmin, (req, res) => {
    ...
    db.run(sql, [usuario_a_eliminar], function(err) {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/admin/change-role', checkSession, checkAdmin, (req, res) => {
    ...
    const sql = 'UPDATE usuarios SET rol = ? WHERE usuario = ?';
    db.run(sql, [nuevo_rol, usuario_a_cambiar], function(err) {
        if (err) {
            return res.status(500).json({ error: err.message });
        }
...
// --- API de PACIENTES ---
app.get('/api/buscar-pacientes-fragmento', checkSession, (req, res) => {
    try {
        ...
        const normalizar = (txt) => txt ? txt.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase().replace(/\s+/g, ' ').trim() : '';
        const q = normalizar(query);

        // Buscar con normalización completa en ambos lados
        const sql = `SELECT dni, nombre FROM pacientes WHERE REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(nombre), 'Á', 'A'), 'É', 'E'), 'Í', 'I'), 'Ó', 'O'), 'Ú', 'U'), 'Ñ', 'N'), 'Ü', 'U'), 'À', 'A'), 'È', 'E'), 'Ò', 'O'), 'á', 'a'), 'é', 'e'), 'í', 'i'), 'ó', 'o'), 'ú', 'u'), 'ñ', 'n'), 'ü', 'u'), 'à', 'a'), 'è', 'e'), 'ò', 'o') LIKE ?`;
        const params = [q + '%'];
...
app.get('/api/buscar-paciente', checkSession, (req, res) => {
    const { query } = req.query; 
    let sqlQuery = `
        SELECT p.*, 
               json_group_array(
    ...
    if (soloNumeros.length > 0 && /^\d+$/.test(query)) {
        sqlQuery += ` WHERE REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(p.dni), 'F', ''), 'M', ''), '-', ''), '_', ''), ' ', '') LIKE ?`;
        ...
        sqlQuery += ' WHERE REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(p.nombre), \'Á\', \'A\'), \'É\', \'E\'), \'Í\', \'I\'), \'Ó\', \'O\'), \'Ú\', \'U\'), \'Ñ\', \'N\'), \'Ü\', \'U\'), \'À\', \'A\'), \'È\', \'E\'), \'Ò\', \'O\'), \'á\', \'a\'), \'é\', \'e\'), \'í\', \'i\'), \'ó\', \'o\'), \'ú\', \'u\'), \'ñ\', \'n\'), \'ü\', \'u\'), \'à\', \'a\'), \'è\', \'e\'), \'ò\', \'o\') LIKE ?';
...
app.post('/api/nuevo-paciente', checkSession, checkAdmin, upload.array('archivos', 5), async (req, res) => {
    ...
    const telefonoNorm = telefono ? telefono.trim() : '';
    ...
    const titularDniNorm = titular_dni ? titular_dni.trim() : '';
    const titularNombreNorm = normalizarNombre(titular_nombre);

    const sql = `INSERT INTO pacientes (dni, nombre, condicion, telefono, direccion, localidad, fecha_nacimiento, tipo_afiliado, sexo, vinculo_titular, observaciones, titular_dni, titular_nombre) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
...
app.post('/api/editar-paciente/:dni', checkSession, checkAdmin, upload.array('archivos', 5), async (req, res) => {
    ...
    const sql = `UPDATE pacientes 
         SET nombre = ?, condicion = ?, telefono = ?, direccion = ?, localidad = ?, fecha_nacimiento = ?, tipo_afiliado = ?, sexo = ?, vinculo_titular = ?, observaciones = ?, titular_dni = ?, titular_nombre = ?
         WHERE dni = ?`;
...
app.post('/api/nueva-prestacion', checkSession, checkAdmin, (req, res) => {
    ...
    db.get(`SELECT dni FROM pacientes WHERE REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(UPPER(dni), 'F', ''), 'M', ''), '-', ''), '_', ''), ' ', '') LIKE ?`, [`%${soloNumeros}`], (err, row) => {
        if (err) return res.status(500).json({ error: err.message });
        ...
        db.serialize(() => {
            ...
            const sql = `INSERT INTO prestaciones (paciente_dni, fecha, prestador, prestacion) VALUES (?, ?, ?, ?)`;
            ...
            prestaciones.forEach((p, idx) => {
                ...
                db.run(sql, [dniReal, fechaPrestacion, prestador, descripcionPrestacion], function(err) {
...
app.put('/api/prestacion/:id', checkSession, checkAdmin, (req, res) => {
    ...
    const sql = 'UPDATE prestaciones SET fecha = ?, prestador = ?, prestacion = ? WHERE id = ?';
    db.run(sql, [fechaFinal, prestador, prestacion, id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        if (this.changes === 0) return res.status(404).json({ message: 'Prestación no encontrada.' });
...
app.delete('/api/prestacion/:id', checkSession, checkAdmin, (req, res) => {
    const id = req.params.id;
    const sql = 'DELETE FROM prestaciones WHERE id = ?';
...
app.delete('/api/paciente/:dni', checkSession, checkAdmin, (req, res) => {
    ...
    db.all('SELECT ruta_archivo FROM archivos_paciente WHERE paciente_dni = ?', [dni], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        ...
        db.serialize(() => {
            db.run('DELETE FROM archivos_paciente WHERE paciente_dni = ?', dni);
            db.run('DELETE FROM prestaciones WHERE paciente_dni = ?', dni);
            db.run('DELETE FROM pacientes WHERE dni = ?', dni, function(err) {
                if (err) return res.status(500).json({ error: err.message });
                if (this.changes === 0) return res.status(404).json({ message: 'Paciente no encontrado' });
...
https.createServer(credentials, app).listen(port, () => {
    console.log(`Servidor seguro por HTTPS en puerto ${port}`);
});